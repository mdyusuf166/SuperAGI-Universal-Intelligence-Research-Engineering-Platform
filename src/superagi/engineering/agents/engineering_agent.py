...
class EngineeringAgent(BaseAgent):
 ...
 async def run(self,task,context):
  result=EngineeringPipeline().run(context.values["problem"],context.values.get("requirements",[]),context.values.get("constraints",[]));return AgentResult(success=result["status"]=="COMPLETED",confidence=.3,summary="Concise conceptual engineering report; no hardware execution.",output={"status":result["status"],"report":result.get("report",""),"provenance":result.get("design").provenance if result.get("design") else []},limitations=["Simulation/design proposal only; not certification."])
